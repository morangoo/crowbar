    ...
    match request.send().await {
        Ok(resp) => match resp.text().await {
            Ok(html) => {
                ...
                let img_selector = Selector::parse("img#result_0_image").unwrap();
                let name_selector = Selector::parse("span#result_0_name").unwrap();
                let img_html = if let Some(img) = document.select(&img_selector).next() {
                    if let Some(src) = img.value().attr("src") {
                        format!("<img src=\"{}\" />", src)
                    } else {
                        "<h1>Image not found</h1>".to_string()
                    }
                } else {
                    "<h1>Image not found</h1>".to_string()
                };
                let name_html = if let Some(name) = document.select(&name_selector).next() {
                    ...
                    format!("<div><b>{}</b></div>", name_text)
                } else {
                    "<div><b>Name not found</b></div>".to_string()
                };
                RawHtml(format!("{}{}", name_html, img_html))
            },
